...
[The Latent factor multi-species occupancy model is described in detail here](https://doserlab.com/files/spoccupancy-web/articles/factormodels#basic-model-description)



### Tamshiyacu Tahuayo data

Here we use the tables: 
- PER-003_BD_ACRCTT_T0.xlsx
- PER-002_BD_ACRTT-PILOTO.xlsx
...
# extract FLII
Per_full_sf$FLII <- terra::extract(FLII2016, Per_full_sf)[,2]
str(Per_full_sf$FLII)


# mapview(full_sites_14_15_16_sf, zcol = "in_AP", burst = TRUE)

# Transform coord to Lambert Azimuthal Equal-Area
Per_full_sf_UTM <- st_transform(Per_full_sf, "EPSG:10603")
...
mcmc_intervals(out.sp$beta.samples[,67:88] , point_est = "mean",
               prob = 0.75, prob_outer = 0.95) + 
  geom_vline(xintercept = 0, color = "red", linetype = "dashed", size = 0.5)
